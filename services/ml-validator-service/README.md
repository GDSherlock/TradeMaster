...
2. `model unavailable`：检查 `models/<champion_version>/model.joblib` 是否存在。
3. 漂移触发重训失败：查看 `logs/ml-validator.log`，并确认 DB 写入权限。
4. 再校准无晋升：检查 `coverage` 是否落在 `[0.15,0.60]`。
5. 回滚 champion：将 `signal_ml_runtime_state.champion_version` 指回 last-good 版本。
